...
class Boid {
  ...
  void run(ArrayList<Boid> boids) {
    ...
    if (insideC) {
      ...
      //applyForce(seek(regionFlow));
    } else {
      flock(boids);
    }
    float mappedX = constrain(map(pos.x, 0, 1280, 0, 512),1,511);
    float mappedY = constrain(map(pos.y, 0, 720, 0, 416),1,415);
    mappedX=constrain(mappedX, 0, 511);
    mappedY=constrain(mappedY, 0, 423);
    int px = floor(mappedX/resolutionx );
    int py = floor(mappedY/resolutiony );
    if (frameCount>1) {
      PVector regionFlow = opencv.getAverageFlowInRegion(px*resolutionx, py*resolutiony, resolutionx, resolutiony);
  
      regionFlow.mult(100);
      pushMatrix();
      translate(map(px*resolutionx,1,511,0,width),map(py*resolutiony,0,423,0,height));
      strokeWeight(10);
      stroke(255,0,0);
      line(0,0,regionFlow.x, regionFlow.y);
      popMatrix();
      //println(regionFlow);
  ...
  void render() {
    ...
    rotate(theta);
    if (this.clrState == 1) {
      butterflyW.displayHidden(0, 0, this.size, color(0));
      butterfly.display(0, 0, this.size, this.clr);
    } else {
      butterfly.displayHidden(0, 0, this.size, color(0));
      butterflyW.display(0, 0, this.size, this.clrW);
    }
    popMatrix();
  }
