...
class Boid {
  ...
  void repel(PVector target) {
    ...
    //if (d<170){
    d = constrain(d, 1.0, 20.0);                        // Limiting the distance to eliminate "extreme" results for very close or very far objects
    ...
    //// Steering = Desired minus this.this.vel
    //PVector steer = PVector.sub(force, this.vel);
    //println(steer);
  ...
  void friction(){
    if(this.vel.mag()>10){
      PVector force = this.vel.copy().mult(-0.9);
      applyForce(force);
    }
  }
